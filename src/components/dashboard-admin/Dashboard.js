...
const Dashboard = ({children}) =>{
    ...
    useEffect(()=>{
        ...
        if(getCookie('auth-keycloak')==null)
        {
            router.push('/admin/login');
        }else{
            console.log("PAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
            console.log('Bearer ' + getCookie('auth-keycloak'));
            axios
                /*.get("http://localhost:8082/api/superuser/token" ,*/
                .get("https://tehranch.com/api/superuser/token" ,
                    {
                        headers : {
